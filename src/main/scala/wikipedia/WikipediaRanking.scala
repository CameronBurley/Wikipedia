...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd
    .aggregate(0)((acc: Int, curr: WikipediaArticle) => {
      (if curr.mentionsLanguage(lang) then acc + 1 else acc + 0)
    }, _ + _)
   ...
   *   several seconds.
   */
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs.par.map { lang =>
